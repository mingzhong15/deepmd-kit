...
class DOSFitting ():
    ...
    def compute_output_stats(self, 
        ...
        all_stat
                ...
                all_stat['dos'] of shape n_sys x n_batch x n_frame
                can be prepared by model.make_stat_input
        """
        self.bias_dos = self._compute_output_stats(all_stat, rcond = self.rcond)
    ...
    def _compute_output_stats(self, all_stat, rcond = 1e-3):
        ...
        for ss in range(len(data)):
            ...
            sys_dos = np.append(sys_dos, np.average(sys_data))
        ...
        dos_shift,resd,rank,s_value \
            = np.linalg.lstsq(sys_tynatom, sys_dos, rcond = rcond)
        return dos_shift    
    ...
    def build (self, 
        ...
        with tf.variable_scope('fitting_attr' + suffix, reuse = reuse) :
            ...
            t_daparam = tf.constant(self.numb_aparam, 
                                    name = 'daparam', 
                                    dtype = tf.int32)
            t_numb_dos = tf.constant(self.numb_dos,
                                    name = 'numb_dos',
                                    dtype = tf.int32)
        ...
        for type_i in range(self.ntypes):
            ...
            if bias_dos is None :
                ...
                type_bias_ados = bias_dos[type_i]
            ...
            final_layer = one_layer(layer, self.numb_dos, activation_fn = None, name='final_layer_type_'+str(type_i)+suffix, reuse=reuse, seed = self.seed, precision = self.fitting_precision, trainable = self.trainable[-1])

            final_layer = tf.reshape(final_layer, [tf.shape(inputs)[0], natoms[2+type_i], self.numb_dos])
